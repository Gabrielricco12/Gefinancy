...
import { 
  ...
  CalendarRange, Plus, ArrowUpCircle, ArrowDownCircle, Wallet
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      // 1. Buscar Extrato do Mês (Para a lista e cards pequenos)
      const monthlyData = await getStatement(user.id, start, end);
      setStatement(monthlyData);

      // Calcular Fluxo do Mês (Apenas visualização mensal)
      let mIncome = 0;
      let mExpense = 0;
      ...
      // 2. CALCULAR SALDO TOTAL (Histórico Completo: Receitas - Despesas + Saldo Inicial)
      
      // A. Buscar Saldos Iniciais das Contas
      const { data: accountsData } = await supabase
        .from('accounts')
        .select('initial_balance');
      const totalInitial = accountsData?.reduce((acc, curr) => acc + Number(curr.initial_balance || 0), 0) || 0;

      // B. Buscar Todas as Receitas (Incomes)
      const { data: allIncomes } = await supabase
        .from('incomes')
        .select('amount');
      const totalIncomes = allIncomes?.reduce((acc, curr) => acc + Number(curr.amount || 0), 0) || 0;

      // C. Buscar Todas as Despesas (Expenses) - Total Amount (Comprometido)
      const { data: allExpenses } = await supabase
        .from('expenses')
        .select('total_amount');
      const totalExpenses = allExpenses?.reduce((acc, curr) => acc + Number(curr.total_amount || 0), 0) || 0;
  ...
  return (
    <MainLayout>
      <header className="pt-safe px-6 pb-2 flex justify-between items-center z-20 relative bg-gray-50/0 backdrop-blur-sm mt-2">
        ...
        <button onClick={handleLogout} className="p-2 bg-white rounded-full shadow-sm text-red-500 hover:bg-red-50 transition border border-red-100">
          <LogOut size={20} />
        </button>
      </header>
        ...
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            {loading ? (
              ...
              <>
                {/* CARD DE SALDO GLOBAL (Cálculo Real) */}
                <div className="w-full bg-gradient-to-br from-blue-600 to-indigo-600 p-6 rounded-3xl shadow-xl shadow-blue-500/20 text-white relative overflow-hidden mb-6">
                  ...
                  <div className="mt-2 text-xs text-blue-100 opacity-80">
                    Receitas - Despesas + Saldo Inicial
                  </div>
                </div>

                {/* CARDS DE FLUXO MENSAL */}
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-white/50 shadow-sm flex flex-col justify-between">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-sm text-gray-600 font-medium">Gastos (Mês)</span>
                    </div>
                        ...
                        <div key={item.id} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100/50">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-500">{item.category?.name || 'Geral'} • {item.account?.name}</p>
                            </div>
                          </div>
