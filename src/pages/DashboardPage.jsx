...
import { 
  Bell, Eye, EyeOff, ArrowDown, ArrowUp, 
...
// ... (Manter variantes slideVariants, modalVariants, getIconByCategory, getColorByCategory, swipePower) ...
// Vou omitir as constantes visuais aqui para economizar espaço, mas MANTENHA ELAS no arquivo.
...
const getIconByCategory = (slug) => <ShoppingCart size={20} className="text-white" />; // Simplificado para exemplo, mantenha o seu completo
const getColorByCategory = (slug) => 'bg-blue-500'; // Simplificado
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const data = await getStatement(user.id, start, end);
      setStatement(data);
  ...
  return (
    <MainLayout>
      ...
      <header className="pt-safe px-6 pb-2 flex justify-between items-center z-20 relative bg-gray-50/0 backdrop-blur-sm mt-2">
        ...
        <button className="p-2 bg-white rounded-full shadow-sm text-gray-600 hover:bg-gray-100 transition">
          <Bell size={20} />
        </button>
      </header>
              ...
              <>
                ...
                <div className="w-full bg-gradient-to-br from-blue-500 to-cyan-400 p-6 rounded-3xl shadow-xl shadow-blue-500/20 text-white relative overflow-hidden mb-6">
                   ...
                   <div className="flex justify-between items-start mb-2">
                    <span className="text-blue-50 font-medium text-sm">Saldo Previsto</span>
                ...
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-white/50 shadow-sm flex flex-col justify-between">
                    ...
                    <span className="text-lg font-bold text-gray-800">{showBalance ? formatCurrency(summary.expense) : '••••'}</span>
                  </div>
                  <div className="bg-white/60 backdrop-blur-md p-4 rounded-2xl border border-white/50 shadow-sm flex flex-col justify-between">
                    <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-lg font-bold text-gray-800">{showBalance ? formatCurrency(summary.income) : '••••'}</span>
                  </div>
                </div>
                      ...
                      statement.map((item) => (
                        <div key={item.id} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm border border-gray-100/50">
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-full flex items-center justify-center shadow-sm ${getColorByCategory(item.category?.slug)}`}>
      ...
      {/* Menu FAB e Modais (Mantidos aqui pois interagem com o estado da página) */}
      <AnimatePresence>
        {isMenuOpen && (
