...
export default function ExpenseForm({ onClose, onSuccess }) {
  ...
  // Carregar dados (Contas + Cartões + Categorias da Família)
  useEffect(() => {
    async function loadData() {
      try {
        const [accRes, cardRes, catRes] = await Promise.all([
          // REMOVIDO: .eq('user_id', user.id) -> RLS filtra por família
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        userId: user.id, // Criação continua vinculada a quem está logado
