...
export default function IncomeForm({ onClose, onSuccess }) {
  ...
  // Carregar dados da Família (Sem filtro de user_id)
  useEffect(() => {
    async function loadAux() {
      // REMOVIDO: .eq('user_id', user.id)
      const { data: acc } = await supabase.from('accounts').select('*').order('name');
      const { data: cat } = await supabase.from('categories').select('*').eq('type', 'income').order('name');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createIncome({
        userId: user.id, // Cria com seu ID, mas visível para a família
        accountId,
        categoryId,
