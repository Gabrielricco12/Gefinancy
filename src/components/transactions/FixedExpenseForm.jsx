...
import { addMonths, format, getDate, parseISO } from 'date-fns'; // Adicionado getDate e parseISO
...
export default function FixedExpenseForm({ onClose, onSuccess }) {
  ...
  // --- CÁLCULO INTELIGENTE DA DATA FINAL ---
  // Se o dia de início (ex: 09) for DEPOIS do dia de vencimento (ex: 05),
  // a primeira parcela conta apenas no mês seguinte.
  const calculateEndDate = () => {
    ...
    // Se a data de início (hoje) já passou do dia de vencimento deste mês,
    // a contagem de 12 meses começa tecnicamente no mês que vem.
    // Lógica: (Meses - 1) é o padrão matemático para "inclusive", 
    // mas se pulamos o primeiro mês, somamos +1 no offset.
    
    let offset = totalMonths - 1; 
    
    if (startDay > dueDayNum) {
      offset += 1; // Pula este mês, empurra o final pra frente
    }
