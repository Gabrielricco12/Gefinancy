...
export default function FixedExpenseForm({ onClose, onSuccess }) {
  ...
  const [isInfinite, setIsInfinite] = useState(true); // true = Aluguel, false = Empréstimo
  ...
  const endDate = !isInfinite && totalMonths > 0 
    ? addMonths(new Date(startDate), totalMonths - 1) 
    : null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await FixedExpensesService.create({
        ...
        startDate: startDate,
        // Envia null se for infinito, ou o número se for finito
  ...
  return (
    <div className="bg-white w-full rounded-t-3xl shadow-2xl flex flex-col h-[90vh] pb-safe">
      ...
      <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-6 py-6 space-y-6">
        ...
        {/* Campos Condicionais (Aparecem só se for Determinado) */}
        {!isInfinite && (
        ...
        {endDate && (
           <div className="p-3 bg-indigo-50 rounded-2xl border border-indigo-100 text-sm text-indigo-700 flex items-center gap-2">
             <Hourglass size={16} />
             <span>Essa despesa terminará em <strong>{format(endDate, 'MMMM yyyy', { locale: ptBR })}</strong>.</span>
           </div>
        )}
